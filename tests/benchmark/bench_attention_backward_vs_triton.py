...
def is_blackwell():
    # Treat SM >= 10 as Blackwell-class for tuning decisions.
...
def _attn_fwd(
    ...
    if STAGE & 1:
        acc, l_i, m_i = _attn_fwd_inner(
            acc, l_i, m_i, q, desc_k, desc_v, offset_y, dtype, start_m, qk_scale,
            BLOCK_M, HEAD_DIM, BLOCK_N, 4 - STAGE, offs_m, offs_n, N_CTX, warp_specialize, IS_HOPPER
        )
    if STAGE & 2:
        acc, l_i, m_i = _attn_fwd_inner(
            acc, l_i, m_i, q, desc_k, desc_v, offset_y, dtype, start_m, qk_scale,
            BLOCK_M, HEAD_DIM, BLOCK_N, 2, offs_m, offs_n, N_CTX, warp_specialize, IS_HOPPER
        )
...
def _attn_bwd(
    ...
    if CAUSAL:
        ...
        dk, dv = _attn_bwd_dkdv(
            dk, dv, Q, k, v, sm_scale, DO, M, D, stride_tok, stride_d, H, N_CTX,
            MASK_BLOCK_M1, BLOCK_N1, HEAD_DIM, start_n, start_m, num_steps, MASK=True
        )
    ...
    if CAUSAL:
        ...
        dq = _attn_bwd_dq(
            dq, q, K, V, do, m, D, stride_tok, stride_d, H, N_CTX,
            BLOCK_M2, MASK_BLOCK_N2, HEAD_DIM, start_m,
            end_n - num_steps * MASK_BLOCK_N2, num_steps, MASK=True
        )
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale, warp_specialize=True):
        ...
        # Host tensor descriptors are fragile across Triton versions.
        # Disable them here for robustness; use raw pointers instead.
        use_host_desc = False
        if use_host_desc and supports_host_descriptor() and not (is_hopper() and warp_specialize):
    ...
    def backward(ctx, do):
        ...
        NUM_WARPS = 4
        NUM_STAGES = 5
        # Blackwell (SM 12.x) hits shared-memory OOR at HEAD_DIM=128.
        if torch.cuda.get_device_capability()[0] >= 12 and ctx.HEAD_DIM >= 128:
            NUM_STAGES = 4
...
def bench_attention_vs_triton(
    ...
    if backend == "cutile":
        ...
        # Match Triton reference behavior: enable warpspec only on Blackwell,
        # or on Hopper for non-causal forward. Keep it off for backward-only.
        warp_specialize = False
        if mode != "bwd":
            warp_specialize = is_blackwell() or (is_hopper() and not causal)

        def fwd_fn():
            # torch.autograd.Function.apply doesn't accept keyword arguments
    ...
    try:
        ms = triton.testing.do_bench(build_fn())
    except Exception as e:
        # Retry Triton with warpspec disabled if it was enabled.
        if backend == "triton" and "warp_specialize" in locals() and warp_specialize:
            warp_specialize = False
            ...
            try:
                ...
                if os.environ.get("TRITON_BENCH_DEBUG") == "1":
                    print(f"[Triton bench failed] {e2}")
                return float("nan")
        else:
            if os.environ.get("TRITON_BENCH_DEBUG") == "1":
