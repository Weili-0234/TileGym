...
Architecture:
  Input: q[B,HQ,S,D], k[B,G,S,D], v[B,G,S,D], gates[B,HQ,S,3]
    ...
    ├─ Stage 5: Sliding Window Attention (cutile kernel) → o_swa
...
# Adapted from attention_sink.py BANDWIDTH logic + fmha_fwd_kernel_with_lse
# ============================================================================


@ct.kernel(occupancy=2)
def nsa_sliding_window_kernel(
    Q,    # [B, HQ, S, D]
    K,    # [B, G, S, D]
    V,    # [B, G, S, D]
    Out,  # [B, HQ, S, D]
...
def sliding_window_attention(q, k, v, window_size, scale=None):
    ...
    if _should_disable_autotune():
        ...
        ct_experimental.autotune_launch(
            stream,
            grid_fn=lambda cfg: (math.ceil(S / cfg.TILE_M), B * HQ, 1),
            kernel=nsa_sliding_window_kernel,
            args_fn=lambda cfg: (
                q, k, v, o, scale, D, HQ, S,
                cfg.TILE_M, cfg.TILE_N, query_group_size, window_size,
            ),
